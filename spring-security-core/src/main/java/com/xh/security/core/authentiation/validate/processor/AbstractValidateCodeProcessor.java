...
public abstract class AbstractValidateCodeProcessor implements ValidateCodeProcessor{
    ...
    @Override
    public void validate(HttpServletRequest request, HttpServletResponse response) throws AuthenticationBusinessException {
        String inputCode = request.getParameter(inputCodeKey);
        if (StringUtils.isBlank(inputCode)) {
            throw new AuthenticationBusinessException("验证码不能为空");
        }
        //获取用户的缓存codeKey
        ...
        if (validateCode.expire()) {
            ...
            throw new AuthenticationBusinessException("验证码已失效");
        }
        if (!StringUtils.equalsIgnoreCase(validateCode.getCode(), inputCode)) {
            throw new AuthenticationBusinessException("验证码不正确");
        }
        authCache.remove(onlyCacheCodeKey);
    }
