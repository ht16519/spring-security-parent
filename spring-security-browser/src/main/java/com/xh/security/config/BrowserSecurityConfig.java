...
import com.xh.security.authentiation.config.AbstractAuthenticationConfig;
import com.xh.security.authentiation.config.SmsCodeAuthenticationSecurityConfig;
import com.xh.security.authentiation.config.ValidateCodeSecurityConfig;
...
//        jdbcTokenRepository.setCreateTableOnStartup(true);  //启动时创建记住我的数据库表
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.apply(validateCodeSecurityConfig)                  //将短信验证码认证逻辑配置加入
                .and()
                .apply(smsCodeAuthenticationSecurityConfig)        //验证码校验逻辑配置加入
                .and()
                .rememberMe()                                                   //配置记住我功能
